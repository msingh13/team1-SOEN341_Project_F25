...
exports.list = async (req, res) => {
  try {
    ...
    const perPage = Math.min(50, Math.max(1, parsePositiveInt(req.query.perPage || req.query.limit || '12', 12)));
    ...
    if (dateFrom && !isIsoDateString(dateFrom)) {
      return bad(res, 'dateFrom must be an ISO date string (YYYY-MM-DD or ISO datetime)');
    }
    if (dateTo && !isIsoDateString(dateTo)) {
      return bad(res, 'dateTo must be an ISO date string (YYYY-MM-DD or ISO datetime)');
    }
    ...
    // query DB (matches your db/queries/events signatures)
    ...
    return res.status(500).json({ code: 'INTERNAL_ERROR', message: 'Failed to list events', details: e.message });
  }
};
