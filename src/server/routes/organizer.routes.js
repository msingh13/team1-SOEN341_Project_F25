...
// ✅ import the function, not the module object
 ...
 * Uses remaining = capacity - (claimed + checked_in)
 */
router.get("/org/events", authenticateToken, async (req, res) => {
  ...
  try {
    // Your organizers table: prefer org_id if present; fallback to o.id
    const orgResult = await pool.query(
      `SELECT COALESCE(org_id, id) AS org_id
         FROM organizers
        WHERE user_id = $1
        LIMIT 1`,
      [userId]
 ...
 * Organizer analytics for a single event they own.
 */
