...
// --- Organizer approvals ---
router.get("/organizers/pending", authenticateToken, requireAdmin, async (_req, res) => {
  try {
...
router.post("/organizers/:userId/approve", authenticateToken, requireAdmin, async (req, res) => {
...
router.post("/organizers/:userId/reject", authenticateToken, requireAdmin, async (req, res) => {
  const userId = Number(req.params.userId);
  if (!Number.isFinite(userId)) {
    return res.status(400).json({ code: "BAD_REQUEST", message: "Invalid user id" });
  }
...
// --- Event moderation ---
router.get("/events/pending", authenticateToken, requireAdmin, async (_req, res) => {
  try {
    const { rows } = await pool.query(
...
router.post("/events/:id/publish", authenticateToken, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) {
      const cur = await pool.query(`SELECT status FROM events WHERE id = $1`, [eventId]);
...
router.post("/events/:id/reject", authenticateToken, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) {
      return res.status(404).json({ code: "NOT_FOUND", message: "Event not found or immutable" });
    }
    return res.json({ ok: true, event: rows[0] });
  } catch (e) {
