...
export default function OrganizationsPage() {
  ...
  const activeOrg = useMemo(
    () => orgs?.find((o) => o.id === membersOpenId) ?? null,
    [orgs, membersOpenId]
  );
  ...
  async function refresh() {
    try {
      ...
      setToast({
        type: "error",
        msg: e?.message ?? "Failed to load organizations",
      });
    }
  }
  ...
  async function onAssign(userId: number, role: Role) {
    if (!activeOrg) return;
    try {
      await assignRole(activeOrg.id, userId, role);
      await refresh();
      setMembersOpenId(activeOrg.id); // reopen drawer
  ...
  async function onRemove(userId: number) {
    if (!activeOrg) return;
    try {
      await removeRole(activeOrg.id, userId);
      await refresh(); // 🔑 re-fetch so members list is up to date
      setMembersOpenId(activeOrg.id);
      showSuccess("Member removed");
    } catch (err: any) {
      // last-admin safeguard errors will show here
    ...
    <div className="container" style={{ padding: 24 }}>
      <header className="card-header" style={{ marginBottom: 12 }}>
        <div>
          ...
          <p className="muted">
            Create, edit, and manage roles for each organization.
          </p>
        </div>
        <button
          className="btn"
      ...
      {toast && (
        <div
          ...
          style={{
            ...
            border: `1px solid ${
              toast.type === "success" ? "#1f8a4c" : "#a43b3b"
            }`,
        ...
        >
          <div style={{ padding: 10 }}>{toast.msg}</div>
        </div>
      )}
      ...
      {activeOrg && (
        ...
        >
          ...
          >
            <button className="btn" onClick={() => setMembersOpenId(null)}>
              Close
            </button>
          </footer>
        </dialog>
...
function AssignForm({
  ...
  orgId: number;
  onAssign: (userId: number, role: Role) => void | Promise<void>;
}) {
